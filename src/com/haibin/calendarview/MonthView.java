...
/**
 * 月视图基础控件,请使用 MonthView替换，没有任何不同，只是规范命名 pleased using MonthView replace
 * BaseCalendarCardView Created by huanghaibin on 2017/11/15.
 */
...
public abstract class MonthView extends View implements View.OnClickListener {
	...
	void setup(CustomCalendarViewDelegate delegate) {
		...
		this.mSelectedPaint.setColor(0xff94caee); // 选中的圈圈的颜色
			...
			for (int j = 0; j < 7; j++) {
				...
				if (mDelegate.mSchemeDate != null && mDelegate.mSchemeDate.contains(calendar)) {
					...
					if (isSelected) {
						...
						// 将画笔设置为标记颜色
						// calendar.getSchemeColor() != 0 ?
						// calendar.getSchemeColor() : mSchemeColor
	...
	public void onClick(View v) {
		if (isClick) {
			...
			if (calendar != null) {
				if (!Util.isCalendarInRange(calendar, mDelegate.getMinYear(), mDelegate.getMinYearMonth(),
	...
	private void initCalendar() {
		...
		int firstDayOfWeek = date.get(java.util.Calendar.DAY_OF_WEEK) - 1;// 月第一天为星期几,星期天
																			// ==
																			// 0
		...
		for (int i = 0; i < size; i++) {
			...
			calendarDate.setLunar(
					LunarCalendar.getLunarText(calendarDate.getYear(), calendarDate.getMonth(), calendarDate.getDay()));
	...
	/**
	 * 开始绘制前的钩子，这里做一些初始化的操作，每次绘制只调用一次，性能高效 没有需要可忽略不实现 例如：
	 * 1、需要绘制圆形标记事件背景，可以在这里计算半径 2、绘制矩形选中效果，也可以在这里计算矩形宽和高
	 */
	 ...
	 *            是否选中
	 */
	protected abstract void onDrawText(Canvas canvas, Calendar calendar, int x, int y, boolean hasScheme,
			boolean isSelected);
