...
public class CalendarView extends FrameLayout {
	...
	private void init(Context context) {
		...
		mSelectLayout.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			@Override
	...
	public void showSelectLayout(final int year) {
		...
		mMonthPager.animate().scaleX(0).scaleY(0).setDuration(180).setInterpolator(new LinearInterpolator())
	...
	private void closeSelectLayout(final int position) {
		...
		mWeekBar.animate().translationY(0).setInterpolator(new LinearInterpolator()).setDuration(180)
				.setListener(new AnimatorListenerAdapter() {
					@Override
					public void onAnimationEnd(Animator animation) {
						super.onAnimationEnd(animation);
	...
	public void scrollToYear(int year) {
		mMonthPager.setCurrentItem(12 * (year - mDelegate.getMinYear()) + mDelegate.getCurrentDay().getMonth()
	...
	/**
	 * 内部日期选择，不暴露外部使用 主要是用于更新日历CalendarLayout位置
	 */
	...
	public interface OnDateSelectedListener {
		...
		void onDateSelected(Calendar calendar, Calendar showCalendar, boolean isClick);
	}
}
