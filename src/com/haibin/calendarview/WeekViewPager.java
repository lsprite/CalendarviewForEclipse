...
/**
 * 周视图滑动ViewPager，需要动态固定高度 周视图是连续不断的视图，因此不能简单的得出每年都有52+1周，这样会计算重叠的部分
...
//	}
	...
	void scrollToCurrent() {
		int position = Util.getWeekFromCalendarBetweenYearAndYear(mDelegate.getCurrentDay(), mDelegate.getMinYear(),
	...
	void updateSelected(Calendar calendar) {
		int position = Util.getWeekFromCalendarBetweenYearAndYear(calendar, mDelegate.getMinYear(),
				mDelegate.getMinYearMonth()) - 1;
		setCurrentItem(position);
		WeekView view = (WeekView) findViewWithTag(position);
		if (view != null) {
	...
	private class WeekViewPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			Calendar calendar = Util.getFirstCalendarFromWeekCount(mDelegate.getMinYear(), mDelegate.getMinYearMonth(),
					position + 1);
			WeekView view;
